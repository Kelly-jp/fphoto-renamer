...
- ファイル名クリーンアップ時に `_` / `-` / スペースが連続・混在した場合でも 1 つの区切り文字へ正規化し、禁止文字除去後の `DC-|-Art` のようなケースでも `DC-Art` へ安定して整形されるよう改善
- 除外文字列の適用順を見直し、より長いサフィックスを先に除去するよう修正して、DxO 系サフィックスが重複・部分一致する場合の取りこぼしを防止
- XMP のフィルムシミュレーション取得を改善し、`crs:CameraProfile` / `crs:CameraProfilesProfileName` を解釈して `REALA ACE` などのカメラプロファイル由来の名称を正しく取得できるよう修正
- Lightroom 後処理スクリプトと README の除外設定例に `-DxO_DeepPRIME XD3` を追加し、DxO PureRAW 6 系の出力名を既定設定で除去しやすく改善

## v0.5.2 (2026-03-02)

### 変更点

- ファイル名の禁止文字（例: `|`）を `_` へ置換する挙動を廃止し、禁止文字そのものを削除するよう修正
- 禁止文字削除後にファイル名クリーンアップを適用し、`DC-|-Art` が `DC-Art` になるよう整形を改善
- XMP のフィルムシミュレーション取得を改善し、`crs:LookName` / `crs:Look` 内 `crs:Name` を優先して参照するよう変更
- フィルムシミュレーション値の正規化を追加し、`Camera ` プレフィックス付き値を実際の名称へ整形
- 上記挙動を検証する unit test を追加
...
- CLI の `rename --jpg-input` を複数指定できるようにし、異なるフォルダにある複数JPG/JPEGファイルも一括リネーム可能に変更
- 複数ファイル指定時は1つのリネーム計画として衝突解決（連番サフィックス）を行うよう core を拡張
- 複数ファイル指定かつ `--raw-parent-if-missing` 有効時に、各JPGファイルごとに `JPG親フォルダの1つ上` を RAW 探索ルートとして解決するよう改善
- Lightroom 後処理スクリプト（`.bat` / `.command`）を複数入力パスの受け渡しに対応
- README に複数ファイル入力の実行例を追加
